...
for name in findColumn :
    # 예) 이름, 주소, 유투브 조회수에 대한 인덱스 조회
    findIndex.append(header_list.index(name))

# 엑셀의 워크시트 목록
wsheetList = workbook.sheets()
# csv 파일에 쓰기 위한 인스턴스, 자동 메모리 반납 포함. with ~ as
with open("C:/CookAnalysis/Excel/singer_youtube_0822.csv", "w", newline='') as outFp:
    # csv 모듈의 writer 이용해서 쉽게 쓰기 기능 구현
    csvWriter = csv.writer(outFp)
    # 첫행 헤더를 쓰는 부분
    csvWriter.writerow(findColumn)
    # 2번째 행, 실제 데이터를 입력하는 부분
    for worksheet in wsheetList :
        # 엑셀의 전체 행의 갯수, 보통 구조가 동일
        # 1행의 크기는, 다른 행의 크기와 보통 다 동일
